...
class USDAIngredientValidator:
    ...
    def validate_ingredients(
        ...
        """
        logger.info(f"Validating {len(ingredient_names)} ingredients")
        ...
        logger.info(f"Successfully validated {len(validated_ingredients)} ingredients")
    ...
    def _validate_single_ingredient(self, ingredient_name: str) -> Dict:
        ...
        """
        logger.debug(f"Validating ingredient: {ingredient_name}")
        
        # Search for the ingredient
        search_results = self._search_usda(ingredient_name)
        
        if not search_results:
            logger.warning(f"No results found for ingredient: {ingredient_name}")
            return {
                'name': ingredient_name,
                'brand': 'Generic',
                'calories': 0,
    ...
    def _search_usda(self, query: str, page_size: int = 5) -> List[Dict]:
        ...
        try:
            logger.debug(f"Searching USDA API: {query}")
            ...
            logger.debug(f"Found {len(foods)} results for: {query}")
            return foods
            ...
            logger.error(f"Unexpected error during USDA search: {str(e)}")
            raise
    ...
    def _get_food_details(self, fdc_id: int) -> Optional[Dict]:
        ...
        try:
            logger.debug(f"Fetching food details for FDC ID: {fdc_id}")
            response = requests.get(url, params=params, timeout=self.TIMEOUT)
            response.raise_for_status()
            return response.json()
        except Timeout:
            logger.error(f"USDA API timeout for FDC ID: {fdc_id}")
            return None
        except RequestException as e:
            logger.error(f"Failed to get food details for {fdc_id}: {str(e)}")
            return None
        ...
        for data_type in self.DATA_TYPE_PRIORITY:
            for result in search_results:
                if result.get('dataType') == data_type:
                    logger.debug(f"Selected match with data type: {data_type}")
                    return result
    ...
    def _extract_nutrient(
        ...
        for nutrient in food_nutrients:
            ...
            if nutrient_id_field == nutrient_id:
                ...
                logger.debug(f"Extracted nutrient {nutrient_id}: {value}")
                return float(value) if value else 0
        
        logger.warning(f"Nutrient {nutrient_id} not found in food data")
        return None
    ...
    def _extract_allergens(self, food_data: Dict) -> List[str]:
        ...
        if allergen_list:
            logger.debug(f"Detected allergens: {allergen_list}")
