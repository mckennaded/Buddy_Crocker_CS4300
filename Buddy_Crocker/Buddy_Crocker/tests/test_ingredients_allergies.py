"""
Unit and integration tests for USDA ingredient search and allergen detection features.

Tests the AJAX ingredient search endpoint, allergen detection logic, and frontend integration.
"""
...
class AllergenDetectionTest(TestCase):
    ...
    def setUp(self):
        ...
        self.dairy = Allergen.objects.create(
            name="Dairy",
            category="fda_major_9",
            alternative_names=["milk", "lactose", "casein", "whey", "cream", "cheese"]
        )
    ...
    def test_detect_allergens_partial_word_match(self):
        ...
        # Should detect both soy and dairy (milk)
    ...
    def test_detect_allergens_no_duplicates(self):
        ...
        # Should only have one instance of peanuts allergen
...
class USDASearchEndpointTest(TestCase):
    ...
    def test_search_endpoint_requires_minimum_query_length(self):
        ...
        response = self.client.get(reverse('search-usda-ingredients'), {'q': 'a'})
    ...
    def test_search_endpoint_successful_search(self, mock_usda_api):
        ...
        response = self.client.get(reverse('search-usda-ingredients'), {'q': 'cheddar'})
    ...
    def test_search_endpoint_handles_missing_brand(self, mock_usda_api):
        ...
        response = self.client.get(reverse('search-usda-ingredients'), {'q': 'apple'})
...
class AddIngredientIntegrationTest(TestCase):
    ...
    def test_add_ingredient_duplicate_name_brand_combination(self):
        ...
        # Try to create with same name, different brand (should succeed)
        response = self.client.post(reverse('add-ingredient'), {
            'name': 'Cheese',
            'brand': 'Brand B',
            'calories': 105,
        ...
        # Should have two cheese ingredients with different brands
    ...
    def test_add_ingredient_adds_to_pantry(self):
        """Test that new ingredients are automatically added to user's pantry."""
        self.client.login(username="testuser", password="testpass123")
...
class BrandFieldTest(TestCase):
    ...
    def test_ingredient_unique_together_name_brand(self):
        ...
        with self.assertRaises(IntegrityError):
            Ingredient.objects.create(name='Cheese', brand='Brand A', calories=105)
    ...
    def test_ingredient_ordering_includes_brand(self):
        ...
        # Should be: Apple (Generic), Cheese (Generic), Cheese (Kraft)
